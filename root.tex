...
Diagnostic biomarkers, such as KRT13 (type I cytoskeletal 13, encoding Cytokeratin13---CK13), KRT17 (type I cytoskeletal 17, encoding Cytokeratin 17---CK17), TP53 (encoding p53), and MKI67 (Marker Of Proliferation Ki-67, encoding Ki-67), are applied in routine pathological examination for dysplasia grading (semi quantitative).
...
and collect clinical data and bio-specimen (i.g., pathological specimen, saliva, and serum).
...
A deep-learning-based algorithm might extract clinical features and pathology whole-slide images from \acrshort{ovh} and \acrshort{oscc}.
...
\subsubsection*{Aim 1:} Construct the \acrshort{ovh} and leukoplakia datasets\\[0.5cm]

% 祁力行醫師與吳家佑醫師協助建立北醫OPMD cohort(白斑和OVH為主)、IRB申請及切片染色影像AI開發，收案檢體包含:切片、血漿、口水、冷凍組織、buffy coat，建議收案時間為手術後確認非OSCC再與病患溝通收案
%\end{outline}
...
% Dr Chi
\paragraph*{Cohort} Construct Chi-Mei and TMUH datasets\\[0.5cm]
...
The normal (N) and tumor (T) paired samples were obtained from thirty-five consecutive \acrshort{hnscc} patients at the Department of Dentistry or Head and Neck Surgery Clinics, Kaohsiung Veterans General Hospital (KVGH). 
Written informed consent was obtained from all participants and was also reviewed by the Institutional Review Board (IRB) Committee of KVGH. Tissues were preserved in liquid nitrogen for RNA extraction. 
Second HNSCC cohort was obtained from \acrfull{tmuh}, which contains total 86 patients including 79 men and 7 women with 31 NT-paired samples. Written informed consent was obtained from all participants and was also reviewed by TMU joint-IRB Committee.
Their mean age was 53 year-old, range 30-88 years. The 45 patients were diagnosed in early stages (stage I or II) and 41 in late stages (stage III or IV) of HNSCC. %Clinical samples were carried out with the approval of the Institutional Review Boards. 
Formalin-fixed paraffin embedded tissues (FFPE) were organized into a set of serial tissue microarray (TMA) blocks while each samples contained one normal part and three tumor part derived from different tumor regions to represent its heterogeneity.  All participants underwent a series of clinical evaluations then to be treatment according to NCCN guidelines~\citep{Pfishter2019}. The clinicopathological data was carefully recorded. The clinical staging was registered based on the American Joint Committee on Cancer (AJCC) TNM classification of \acrshort{hnscc}.
...
The fresh frozen tissue samples (KVGH cohort) were maintained with the desired orientation on the cutting block using Optimum Cutting Temperature (OCT) polymer. A thin frozen section was cut at -$15\,^{\circ}\mathrm{C}$ by using a cryostat (Thermo Electron) and deposited on cold indium tin oxide (ITO)-coated conductive glass slides (Bruker Daltonics). The slide and tissue section were then quickly warmed, which let the tissue thaw-mounted onto the slide. Tissue sections were fixed by immersion in 70\% then 95\% ethanol bath each for 30 sec and allowed to dry for 40 min in a desiccator.

%%

\subparagraph{Immunohistochemistry (IHC) Staining}

The blocks of FFPE tissue microarray (from TMUH cohort) were sectioned at 3-\si{\um} slice thickness. Slides were heated for 1 hour at $60\,^{\circ}\mathrm{C}$ to deparaffinize in xylene, rehydrated in graded alcohol, and rinsed in distilled water. Antigen retrieval and immunohistochemistry staining were performed by using a Benchmark XT automated immunohistochemistry system (Ventana Medical Systems). Tissue microarray slides were stained with primary TMSB4X antibodies (Proteintech) at 1:400 dilution. Primary antibodies were detected by using an indirect biotin streptavidin system (iVIEW DAB Detection Kit, Ventana Medical Systems, Inc.) and tissue was counterstained in haematoxylin and eosin. Images of stained slides were retrieved by digital scanner (ScanScope Systems Aperio, CA, USA) and scored manually by one pathologist, Dr. Michael Hsiao.

%%%
\paragraph*{Whole-slide imaging} Construct a Deep Learning Platform of TCGA/TCIA and Taiwan datasets\\[0.5cm]
...
The TCGA's HNSCC images is available at \url{https://github.com/texchi2/HE2RNA_code/blob/master/gdc_manifests/gdc_manifest.2018-06-26_diagnostic_TCGA-HNSC.txt}.
...
\subsubsection*{Aim 3:} Construct a deep learning platform to predict genomics \\[0.5cm]
...
\smartdiagramadd[flow diagram:horizontal]{
 TMU dataset (WSI), HE2RNA, RNA-Seq (Representation)
}{
...
\vspace*{10mm}
\smartdiagramadd[flow diagram:horizontal]{
 TMU dataset (EHR), \acrshort{rnn}, Survival Analysis
}{}%{
%    above of module2/Model (transfer from TCIA),
%    below of module3/Survival Analysis 
%} % addition x2
...
\clearpage
% by Dr Wang's and Dr Chi's Integration and modelling
\subsubsection*{Aim 4:}
%To develop risk prediction model for malignant transformation of \acrshort{ovh} by integrating risk factors identified from all sub-projects: sp1, sp2, and sp3
...
We sincerely want to express our thanks to the \acrshort{ovh} patients who donated their data, and help from TMU Joint Institute Review Board.
...
%%% C006
六、耗材、物品及雜項費用:\\
(1) 凡執行研究計畫所需之耗材、物品(非屬研究設備者)及雜項費用，均可填入本表內。\\
...
\hline
項目名稱   & 說明                        & 單位 & \multicolumn{1}{l|}{數量} & \multicolumn{1}{l|}{單價} & \multicolumn{1}{l|}{金額} & \multicolumn{1}{l|}{備註}                                              \\ \hline
耗材費    & 研究計畫人體試驗委員會審查費            & 次  & 1                       & 12,000                  & 12,000                  & \begin{tabular}[c]{@{}r@{}}審查10,000\\ 修正1,000\\ 結案1,000\end{tabular} \\ \hline
耗材費    & 生物資料庫使用費(切片費)             & 5片 & 19                      & 330                     & 6270                    &                                                                      \\ \hline
耗材費    & 生物資料庫使用費(HE染色費)           & 片  & 92                      & 40                      & 3680                    &                                                                      \\ \hline
耗材費    & 生物資料庫使用費(臨床病歷)            & 每名 & 46                      & 1400                    & 64400                   &                                                                      \\ \hline
耗材費    & 臺北醫學大學數據處研究資料庫檢索費         & 小時 & 4                       & 0                       & 0                       & 目前免費                                                                 \\ \hline
儀器使用費  & Aperio ScanScope 數位病理切片掃描 & 小時 & 10                      & 300                     & 3,000                   &                                                                      \\ \hline
雲端運算   & 國網中心高速電腦GPU人工智慧訓練         & 小時 & 20                      & 689                     & 13,780                  & cm.4xsuper                                                           \\ \hline
...
Taipei Medical University and Academia Sinica & Taiwan & PhD candidate in Translational Medicine & 肄業 & 2013/02 - Present \\ \hline
Taipei Medical University                     & Taiwan & MS in Biomedical Informatics            & 晉讀 & 2011/09 - 2013/01 \\ \hline
...
Taipei Municipal Wanfang Hospital  & Oral and Maxillofacial Surgery   & Director             & 2021/04 - Present \\ \hline
...
\begin{tabular}{|l|l|l|l|}
\hline
\begin{tabular}[c]{@{}l@{}}Programming\\ Procient in: R, Python.\\ Familiar with: Matlab, C++.\end{tabular} & \begin{tabular}[c]{@{}l@{}}Languages\\ Chinese, English (uent)\end{tabular} & \begin{tabular}[c]{@{}l@{}}Sports:\\ Swimming, water rescue, \\ SCUBA diving, bicycle cycling,\\ moutaineering\end{tabular} & \begin{tabular}[c]{@{}l@{}}Mindfulness\\ meditation\end{tabular} \\ \hline
\end{tabular}%
}
\end{table}
\end{CJK*}
